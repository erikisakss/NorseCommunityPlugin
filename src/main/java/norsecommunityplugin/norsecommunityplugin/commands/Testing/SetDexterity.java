...
public class SetDexterity implements CommandExecutor {
    private PlayerProfileManager playerProfileManager;
    private NorseCommunityPlugin plugin;

    public SetDexterity(NorseCommunityPlugin plugin) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length != 2) {
            sender.sendMessage("Usage: /setdexterity <player> <amount>");
            return true;
        }
        ...
        int dexterity;
        try {
            dexterity = Integer.parseInt(args[1]);
        ...
        sender.sendMessage("Set " + target.getName() + "'s Dexterity to " + dexterity + ".");
        return true;
    }
}

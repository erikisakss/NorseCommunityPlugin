...
public class GetArmor implements CommandExecutor {
    private final NorseCommunityPlugin plugin;

    public GetArmor(NorseCommunityPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length != 1) {
            sender.sendMessage("Usage: /getarmor <item_name>");
            return true;
        }
        ...
        ItemStack item = itemManager.createArmorItemStack(itemName);
